/*
DT data acquisition Excel template migration.

Run this script after 01_schema_core.sql and 03_indexes_constraints.sql.
It adds parser metadata to acquisition configs, creates the template registry,
and seeds the first lamination board thickness daily template.
*/
...
IF OBJECT_ID('dbo.DA_AcquisitionConfig', 'U') IS NOT NULL
   AND COL_LENGTH('dbo.DA_AcquisitionConfig', 'ParserType') IS NULL
BEGIN
    ALTER TABLE dbo.DA_AcquisitionConfig
    ADD ParserType NVARCHAR(100) NOT NULL
        CONSTRAINT DF_DA_AcquisitionConfig_ParserType DEFAULT ('standard-table');
END
GO

IF OBJECT_ID('dbo.DA_AcquisitionConfig', 'U') IS NOT NULL
   AND COL_LENGTH('dbo.DA_AcquisitionConfig', 'TemplateId') IS NULL
BEGIN
    ALTER TABLE dbo.DA_AcquisitionConfig
...
IF OBJECT_ID('dbo.DA_AcquisitionConfig', 'U') IS NOT NULL
   AND COL_LENGTH('dbo.DA_AcquisitionConfig', 'ParserOptions') IS NULL
BEGIN
    ALTER TABLE dbo.DA_AcquisitionConfig
    ADD ParserOptions NVARCHAR(MAX) NULL;
END
GO
...
BEGIN
    ...
    (
        ...
        TemplateVersion INT NOT NULL CONSTRAINT DF_DA_ExcelImportTemplate_TemplateVersion DEFAULT ((1)),
        DefinitionJson NVARCHAR(MAX) NOT NULL,
        IsEnabled BIT NOT NULL CONSTRAINT DF_DA_ExcelImportTemplate_IsEnabled DEFAULT ((1)),
        CreateTime DATETIME NOT NULL CONSTRAINT DF_DA_ExcelImportTemplate_CreateTime DEFAULT (GETDATE()),
        UpdateTime DATETIME NOT NULL CONSTRAINT DF_DA_ExcelImportTemplate_UpdateTime DEFAULT (GETDATE()),
        CONSTRAINT PK_DA_ExcelImportTemplate PRIMARY KEY CLUSTERED (Id)
    );
END
GO

IF OBJECT_ID('dbo.DA_ExcelImportTemplate', 'U') IS NOT NULL
   AND NOT EXISTS (SELECT 1 FROM sys.indexes WHERE object_id = OBJECT_ID('dbo.DA_ExcelImportTemplate') AND name = 'UQ_DA_ExcelImportTemplate_Code_Version')
BEGIN
    CREATE UNIQUE NONCLUSTERED INDEX UQ_DA_ExcelImportTemplate_Code_Version
...
BEGIN
    ...
    (
        ...
        CreatedAt DATETIME NOT NULL CONSTRAINT DF_DA_LaminationBoardThicknessDaily_CreatedAt DEFAULT (GETDATE()),
        CONSTRAINT PK_DA_LaminationBoardThicknessDaily PRIMARY KEY CLUSTERED (Id)
    );
END
GO

IF OBJECT_ID('dbo.DA_LaminationBoardThicknessDaily', 'U') IS NOT NULL
   AND NOT EXISTS (SELECT 1 FROM sys.indexes WHERE object_id = OBJECT_ID('dbo.DA_LaminationBoardThicknessDaily') AND name = 'IX_DA_LaminationThickness_BusinessDate')
BEGIN
    CREATE NONCLUSTERED INDEX IX_DA_LaminationThickness_BusinessDate
    ON dbo.DA_LaminationBoardThicknessDaily(BusinessDate, ShiftName, Workshop)
    INCLUDE (ModelNo, BatchNo, Judgment);
END
GO
...
BEGIN
  ...
  "systemFields": ["SourceRow", "FileName", "FullFilePath", "CreateDt"]
}',
        1
    );
END
